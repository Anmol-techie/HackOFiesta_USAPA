...
import { Button, Chip, MenuItem, Paper, TextField } from "@material-ui/core";
...
const AdminDashboard = () => {
  ...
  const [agentRequests] = useAPI("/admin/requests");
  const [productsResponse] = useAPI("/admin/products");
  const [agentResponse] = useAPI("/admin/agents");
  const agents = useInput("");
  const products = useInput("");

  console.log(agentRequests);

  const handleAssign = useCallback(({ event, agent_id, product_id }) => {
    ...
    reader.onload = function () {
      ...
      API({
        url: `/admin/assign?product_id=${product_id}&agent_id=${agent_id}`,
        method: "POST",
        data: {
          unit_barcodes,
        },
  ...
  const handleManualAssign = useCallback(
    (event) => {
      ...
      reader.onload = function () {
        ...
        API({
          url: `/admin/assign?product_id=${products.value}&agent_id=${agents.value}`,
          method: "POST",
          data: {
            unit_barcodes,
          },
    ...
    <Box display="flex" flexDirection="column" padding={4} paddingTop={3}>
                ...
                <Paper>
                  ...
                  >
                    ...
                    >
                      ...
                      <Typography variant="h5" alignSelf="flex-end">
                        {quantity} units
                      </Typography>
                    </Box>
                    <Button
                    ...
                    >
                      ...
                      <input
                        ...
                        accept="text/*"
                        onChange={(event) =>
                          handleAssign({ event, agent_id, product_id })
                        }
                      />
                    </Button>
                  </Box>
      ...
      <Box display="flex">
        <TextField
          ...
          {...agents.bind}
        >
          {agentResponse.data?.map(({ name, uid }) => (
        ...
        <TextField
          ...
          {...products.bind}
        >
          {productsResponse.data?.map(({ name, uid }) => (
            <MenuItem value={uid}>{name}</MenuItem>
          ))}
        </TextField>
        <Button
          variant="outlined"
          style={{ height: 56, marginLeft: 12, width: 360 }}
          component="label"
        >
          Assign
          <input
